...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const loadUserData = useCallback(async (userId: string) => {
    try {
      ...
      const [profileRes, memberRes] = await Promise.race([dataPromise, timeoutPromise]) as any
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(({ data: { session } }) => {
      ...
      if (session?.user) {
        loadUserData(session.user.id).finally(() => {
          if (!cancelled) { clearTimeout(loadingTimeout); setLoading(false) }
        })
      } else {
    ...
    return () => { cancelled = true; clearTimeout(loadingTimeout); subscription.unsubscribe() }
  }, [loadUserData])
